...
export async function POST(request: Request) {
  ...
  try {
    const { files: fileRequests, allowedFileTypes, maxFileSize } = await request.json();
    ...
    if (fileRequests.length > MAX_TOTAL_FILES) {
      return NextResponse.json({ error: `Maximum number of files exceeded ${MAX_TOTAL_FILES} files per upload` }, { status: 400 });
    }

    const presignedUrls = await Promise.all(
      fileRequests.map(async ({ prefix, filename, contentType, fileSize }) => {
        ...
        if (fileSize > maxFileSize) {
          throw new Error(`File size exceeds the maximum allowed size of ${formatBytes(maxFileSize)}`);
        }
